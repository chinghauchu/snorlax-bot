...
| Tauri + TypeScript chat UI; 320px computer pane (file tree + text preview, collapsible; v0.34 12px seam chevron, default collapsed, last choice persisted); agent-pane 288×180 computer preview + desktop Open/Done takeover + Record/Stop/Save as skill; v0.41 desktop composer mic (local whisper.cpp via `POST /v1/transcribe`; editable text; no auto-send); v0.42 leftover mic chrome (paperclip \| field \| muted Mic \| Send; Esc cancel; `Start dictation`); v0.43 iOS composer mic (paperclip \| field \| Mic \| Send; local `POST /v1/transcribe`; editable text; no auto-send; Cancel while recording); v0.44 local TTS Speak on completed LEFT `kind=message` (`POST /v1/speak`; piper; no autoplay); v0.45 mermaid diagrams on completed LEFT `kind=message` (official mermaid / iOS WKWebView; invalid falls back to fence chrome); v0.46 TeX math on completed LEFT `kind=message` (inline `\( \)` / block `$$`; KaTeX; invalid falls back to monospace source); v0.47 stick-to-bottom while streaming (~64px slack; freeze on scroll-up; Send / Regenerates re-arm; 12px muted Jump to latest); v0.48 short multi-bubbles on completed LEFT `kind=message` (split on blank lines after the stream completes); v0.49 optimistic user-RIGHT bubble on Send (chips on the bubble; upload first; block a second Send; reconcile on success; 4xx/5xx restore + muted Couldn't send.); v0.50 Stop generating mid-stream (12px muted Stop at chat-column bottom; client abort; keep partial LEFT as completed; composer focused; hide when idle); v0.51 waiting ··· until first token (after Send until first assistant token; hide when LEFT streams); v0.52 streaming caret on the growing LEFT bubble (12px muted blink after first token until complete / Stop; never with ···); v0.53 Esc = Stop while generating (window Esc / iOS UIKeyCommand Escape; same client abort as Stop; IME and pending widget/approve/connect keep Esc); v0.54 same-turn consecutive LEFT bubbles 6px (different-turn / after tool-widget-approve-connect stay 12px); v0.55 Send muted while generating (muted+disabled until complete / Stop / error / empty; Enter does not send; composer stays editable); v0.56 compact tool traces; v0.57 mid-stream plaintext (live LEFT is plain text; markdown once on complete / Stop); v0.58 Reduce Motion static ··· (on → no pulse; off → pulse); v0.59 focus composer after Stop (Stop/Esc abort; iOS hardware keyboard only); v0.60 Esc = Jump when frozen (chip visible + idle → Jump; generating → Stop); v0.61 Copied feedback (muted 12px/12pt Copied beside Copy for 1.2s; Copy label stays; no toast); v0.62 focus composer after Jump (chip/Esc=Jump returns focus; iOS hardware keyboard only); v0.63 Jump chip fade (120ms opacity; Reduce Motion instant) | Full sandbox computer GUI (browser, terminal, VNC) |
...
| Skills (`SKILL.md` in workspace and/or `SNORLAX_DATA_DIR/skills`) + cron XOR webhook/Slack/GitHub routines (Asia/Taipei cron; GET/POST/PATCH + DELETE 204; webhook URL + Copy; Slack/GitHub fire LEFT 1:1 with `routineName` when that plugin is connected; identity-pane Add/Remove) + desktop teach-a-task (`POST /computer/record` → `POST /skills { name }`) + identity-pane skill markdown editor (`GET/PATCH/DELETE /skills/{sid}`) + blank New skill (`POST /skills { name, body }`; identity-pane Add; record `{ name }` omitted body stays) + 1:1 composer `/` skill autocomplete (existing `GET /skills`; Send injects that SKILL.md; unknown `/foo` and channel `/` stay plain text) + v0.35 new agents copy seed SKILL.md (teammates + routines + memory) into their workspace; startup backfill for existing agents; v0.36 per-agent `memory/{agentId}` facts (`remember` / `forget`; tool line Remembered / Forgot) + v0.37 agent-pane Memory list (GET/DELETE `/memory`; Remove exact-forgets; empty `No memories yet.`) + v0.38 leftover Memory pane chrome (`Remove this memory?`; 2-line clamp; desktop hover title; iOS long-press copy; open-pane refetch after Remembered / Forgot) + v0.39 user-shared memory (`scope: user` → `memory/user/`; every agent injects user facts then its own; pane stays agent-only) + v0.40 Settings shared-memory list (`GET/DELETE /v1/memory` on `memory/user/`; Settings lists user facts only; agent pane stays agent-only) + v0.41 desktop local voice dictation (`POST /v1/transcribe`; whisper.cpp; composer mic; no auto-send) + v0.42 leftover desktop mic chrome (Mic left of Send; Esc cancel; `Start dictation` / `Stop dictation`; hint `Transcribing…` / `No speech detected.` / `Microphone is off.`) + v0.43 iOS local voice dictation (same route and hints; paperclip \| field \| Mic \| Send; Cancel while recording; no auto-send) + v0.44 local TTS Speak on LEFT `kind=message` (`POST /v1/speak`; piper; Speak / Stop speaking; no autoplay; never a cloud TTS) + v0.45 mermaid diagrams on completed LEFT `kind=message` (official mermaid / iOS WKWebView; invalid falls back to fence chrome) + v0.46 TeX math on completed LEFT `kind=message` (inline `\( \)` / block `$$`; KaTeX; invalid falls back to monospace source) + v0.47 stick-to-bottom while streaming (~64px slack; Jump to latest while stuck) + v0.48 short multi-bubbles on completed LEFT `kind=message` + v0.49 optimistic user-RIGHT on Send (chips; upload first; block second Send; Couldn't send. on 4xx/5xx) + v0.50 Stop generating mid-stream (12px/12pt muted Stop at chat-column bottom; client abort; keep partial as completed; composer focused) + v0.51 waiting ··· until first token + v0.52 streaming caret on the growing LEFT bubble + v0.53 Esc = Stop while generating + v0.54 same-turn multi-bubble gap (6px/6pt inside a completed LEFT turn; 12px/12pt between turns) + v0.55 Send muted while generating (muted+disabled until complete / Stop / error / empty; Enter does not send; composer stays editable); v0.56 compact tool traces; v0.57 mid-stream plaintext (live LEFT is plain text; markdown once on complete / Stop); v0.58 Reduce Motion static ··· (on → no pulse; off → pulse); v0.59 focus composer after Stop (Stop/Esc abort returns focus; iOS hardware keyboard only); v0.60 Esc = Jump when frozen (chip visible + idle → Jump; generating → Stop); v0.61 Copied feedback (muted 12px/12pt Copied beside Copy for 1.2s; Copy label stays; no toast); v0.62 focus composer after Jump (chip/Esc=Jump returns focus; iOS hardware keyboard only); v0.63 Jump chip fade (120ms opacity; Reduce Motion instant) | Public marketplace / search; event-picker UI |
| OpenAPI for `/v1` | |

Default model on Spark: **70B-class FP8**, swapped via config.

Locked v0.1 / v0.2 / v0.3 / v0.4 / v0.5 / v0.6 / v0.7 / v0.8 / v0.9 / v0.10 / v0.11 / v0.12 / v0.13 / v0.14 / v0.15 / v0.16 / v0.17 / v0.18 / v0.19 / v0.20 / v0.21 / v0.22 / v0.23 / v0.24 / v0.25 / v0.26 / v0.27 / v0.28 / v0.29 / v0.30 / v0.31 / v0.32 / v0.33 / v0.34 / v0.35 / v0.36 / v0.37 / v0.38 / v0.39 / v0.40 / v0.41 / v0.42 / v0.43 / v0.44 / v0.45 / v0.46 / v0.47 / v0.48 / v0.49 / v0.50 / v0.51 / v0.52 / v0.53 / v0.54 / v0.55 / v0.56 / v0.57 / v0.58 / v0.59 / v0.60 / v0.61 / v0.62 / v0.63 (chat layout + agent messaging + collaboration handoff + identity pane + report-back + extra channels + basic tools + computer pane + runtime MCP client + question widgets + skills and cron routines + MCP connect chrome + assistant markdown + MCP Add custom + webhook event listeners + Box computer preview + Box takeover + teach-a-task + create/delete routine UI + skill markdown editor + iOS takeover + iOS Record + composer `/` skill autocomplete + blank New skill + Slack/GitHub inbound listeners + curated plugin catalog + chat attachments + agent-sent attachments + video attachments + watch-video tool + IME Enter + create agent/channel tools + composer clipboard paste + Copy / Regenerate + shell Approve + agent-created routines + user-right bubble wrap + collapse Computer pane + agent bootstrap + durable memory + memory pane + leftover Memory pane chrome + user-shared memory + Settings shared-memory list + desktop local voice dictation + leftover desktop mic chrome + iOS local voice dictation + local TTS Speak on LEFT messages + mermaid diagrams on LEFT `kind=message` + TeX math on LEFT `kind=message` + stick-to-bottom while streaming + short multi-bubbles on LEFT `kind=message` + optimistic user-RIGHT bubble on Send + Stop generating mid-stream + waiting ··· until first token + streaming caret on the growing LEFT bubble + Esc = Stop while generating + same-turn multi-bubble gap + Send muted while generating + compact tool traces + mid-stream plaintext + Reduce Motion static ··· + focus composer after Stop + Esc = Jump when frozen + Copied feedback + focus composer after Jump + Jump chip fade): [docs/specs/v0.1-chat-and-agents.md](docs/specs/v0.1-chat-and-agents.md).
...
- Skills as durable how-to documents a bot can load. **v0.9 first slice:**
  ...
  in the tool list whenever tools are on (not skill-gated). **v0.36:**
  per-agent durable facts (`~/.snorlax-bot/memory/{agentId}`, not the
  sandbox); `remember` / `forget` always in the tool list; injected every
  turn; tool line `Remembered` / `Forgot`. **v0.37:** agent identity pane
  Memory list (GET/DELETE `/v1/agents/{id}/memory`; Remove exact-forgets;
