...
| Tauri + TypeScript chat UI; 320px computer pane (file tree + text preview, collapsible; v0.34 12px seam chevron, default collapsed, last choice persisted); agent-pane 288×180 computer preview + desktop Open/Done takeover + Record/Stop/Save as skill; v0.41 desktop composer mic (local whisper.cpp via `POST /v1/transcribe`; editable text; no auto-send); v0.42 leftover mic chrome (paperclip \| field \| muted Mic \| Send; Esc cancel; `Start dictation`); v0.43 iOS composer mic (paperclip \| field \| Mic \| Send; local `POST /v1/transcribe`; editable text; no auto-send; Cancel while recording); v0.44 local TTS Speak on completed LEFT `kind=message` (`POST /v1/speak`; piper; no autoplay); v0.45 mermaid diagrams on completed LEFT `kind=message` (official mermaid / iOS WKWebView; invalid falls back to fence chrome); v0.46 TeX math on completed LEFT `kind=message` (inline `\( \)` / block `$$`; KaTeX; invalid falls back to monospace source) | Full sandbox computer GUI (browser, terminal, VNC) |
