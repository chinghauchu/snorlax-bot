...
inside that session (201 `{ recording: true }`); `DELETE` that path
stops (204, no file yet). `POST /v1/agents/{id}/skills { name }` writes
SKILL.md from the capture. GET may include `recording` while a session
exists.
