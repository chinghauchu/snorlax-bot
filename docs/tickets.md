...
- Computer pane first slice is v0.6 (D2/B4): desktop file tree + preview over
  ...
  **v0.47:** stick-to-bottom while streaming (~64px slack;
  ...
  (client abort of the in-flight stream; keep the partial
  LEFT text; composer usable again; no auto-restart). Raw HTML still later.
...
- **v0.50:** Stop generating mid-stream. While an assistant
  LEFT turn is streaming, offer Stop generating (Grok Bot
  feel). Client abort of the in-flight stream; keep the
