...
- [D3 — In-stream widgets and attachment chips](https://github.com/chinghauchu/snorlax-bot/issues/8) — **v0.8 first slice:** LEFT question card chrome (desktop + iOS, no extra sheet). **v0.25:** user-right attachment chips (composer paperclip / drop; 56×56 pending image; 36px file chip; GET `attachments` on user-right; `attachmentIds` in that turn). **v0.26:** LEFT `kind=message` reuses that chrome (above markdown, 6px gap; not on tool / widget / Connect / timeline handoff). Video wait.
