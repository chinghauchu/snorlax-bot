...
- [D2 — Computer pane next to chat](https://github.com/chinghauchu/snorlax-bot/issues/12) — v0.6 first slice: 320px right file tree + text preview over the existing `~/.snorlax-bot` workspace (collapsible, default open). **v0.14:** identity-pane 16:10 Bearer PNG preview (288×180). **v0.15:** desktop Open / Done takeover overlay (no VNC / separate window). File-tree column unchanged.
...
- [B4 — Local sandbox computer on the Spark](https://github.com/chinghauchu/snorlax-bot/issues/7) — v0.6 first slice: GET list/read of the existing tool sandbox (same `workspace_for()` roots). **v0.14:** per-agent 1280×800 display; `GET /v1/agents/{id}/computer` `{ hasSandbox, width, height, imageUrl }`; `imageUrl` is Bearer PNG at `/computer/screenshot`. Channel 409. Missing agent 404. **v0.15:** `POST/DELETE /computer/session` (201/204) + `POST .../pointer` `{ x, y, type }` + `POST .../key` `{ key, type }` while the session is up; agent-driven sandbox tools 409. Channel 409. Full B4 (browser, per-agent VNC, 128GB Spark VM) stays later
...
- Computer pane first slice is v0.6 (D2/B4): desktop file tree + preview over
  ...
  **v0.15:** desktop Open / Done takeover (pointer/key in 1280×800). iOS
  stays preview-only. Full sandbox computer GUI (browser, VNC, terminal)
  stays later.
