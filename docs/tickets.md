...
- Attachment chips: **v0.25** user-right composer + transcript (paperclip /
  ...
  second player; desktop/iOS idle). **v0.29:** composing Enter does not
  send; `create_agent` / `create_channel` are the existing 12px muted
  `Created {name}` tool line (no new sheets).
