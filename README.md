...
v0.10 is still small: **named teammates + identity pane + group channel threads
...
(Settings plugins list + `kind=connect` card)**. Header click opens agent identity (PATCH) or
...
See [ROADMAP.md](ROADMAP.md). Short version: v0.10 is named agents plus
...
`kind=connect`), question widgets in the transcript, and
